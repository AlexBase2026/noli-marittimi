...
def normalizza_stringa_porto(valore_cella):
    ...
    if testo_upper in DIZIONARIO_PORTI_CONVERSIONE:
        return DIZIONARIO_PORTI_CONVERSIONE[testo_upper]
    return testo_upper
...
with tab_ricerca:
    ...
    with col1:
        ...
        pol_scelto = st.selectbox("Porto di Partenza (POL)", [""] + [p for p in elenco_pol if p not in ["SCONOSCIUTO", "P.O.D.", ""]])
    with col2:
        elenco_pod = sorted(df_master["POD"].dropna().unique()) if not df_master.empty else []
        pod_scelto = st.selectbox("Porto di Destinazione (POD)", [""] + [p for p in elenco_pod if p not in ["SCONOSCIUTO", "P.O.D.", ""]])
    with col3:
...
# ==========================================
# TAB 2: MULTI-PARSER CORRETTO (.ILOC[0])
# ==========================================
with tab_automatico:
    ...
    if file_caricato is not None:
        try:
            ...
            if st.button("Estrai Solo Noli Base"):
                ...
                for v in riga_porti_alta_raw:
                    val_str = str(v).strip()
                    if pd.notna(v) and val_str != "" and val_str.upper() != "NAN" and "ITALY" not in val_str.upper() and "P.O.D." not in val_str.upper():
                        ultimo_porto_valido = normalizza_stringa_porto(v)
                    riga_porti_alta_pulita.append(ultimo_porto_valido)
                ...
                if trade_file in ["IPBC", "RED SEA", "EAF"]:
                    for idx, row in dati_prezzi.iterrows():
                        ...
                        # MODIFICA: Puntamento corretto alla cella indice 0
                        ...
                        for col_idx in range(1, len(row)):
                            ...
                            lista_tariffe.append({
                                ...
                                "BL": 0.0, "Free_Time": "", "Validità": validita_foglio, "Note": f"Listino {trade_file}", "Origine": "Automatico"
                            })
...
with tab_spese_porto:
    ...
    if not df_master.empty:
        ...
        if st.button("Calcola Automatismi e Applica"):
            ...
            st.success(f"Configurazione completata per POL {pol_selezionato_spese}!")
            st.rerun()
    ...
    with st.form("Form Inserimento Analitico"):
        ...
        if invia_form:
            if man_pol and man_pod and man_carrier:
                ...
                pol_std = normalizza_stringa_porto(man_pol)
                pod_std = normalizza_stringa_porto(man_pod)
